...
async function handleLimitExceeded(userId, limit = 0.10) {
  try {
    ...
    await logUserInteraction(userId, 'limit_exceeded', { dailyLimit: limit })
      .catch(err => logger.error('Failed to log limit exceeded event', { error: err.message }));

    // Retrieve timestamp from mysqldb.js
    const timestamp = await getLatestLimitExceeded(userId);
...
app.get('/user-budget', async (req, res) => {
  try {
    ...
    const remaining = await getRemainingDailyBudget(req.session.userId, DAILY_LIMIT);
    const hasExceeded = await hasExceededDailyLimit(req.session.userId, DAILY_LIMIT);
...
app.get("/user-preferences", async (req, res) => {
  try {
    ...
    if (preferences) {
      res.json({ hasPreferences: true, preferences });
    } else {
      res.json({ hasPreferences: false });
    }
  } catch (err) {
    console.error("Error in /user-preferences:", err);
